...
const MAPPINGS = {
  ...
  developerId: {
    ...
    fun: (devUrl) => (!!devUrl && 'split' in devUrl) ? devUrl.split('id=')[1] : ''
  },
...
function descriptionText (description) {
  if (!!description && 'replace' in description) {
...
function normalizeAndroidVersion (androidVersionText) {
  if (!!androidVersionText && 'split' in androidVersionText) {
    const number = androidVersionText.split(' ')[0];
    if (parseFloat(number)) {
      return number;
    }
  } else {
    console.warn(`[SAMAR_DEBUG] normalizeAndroidVersion: no 'split' for ${androidVersionText}`)
  }
